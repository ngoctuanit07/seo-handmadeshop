<?php
class Sns_Kalolia_Block_Mainmenu extends Mage_Catalog_Block_Navigation {
    ...
    protected function _getMenuItemHtml($category, $level = 0, $isLast = false, $isFirst = false) {
        ...
        if($level==0){
	        ...
	        if($catdetail->getData('kalolia_groupsubcat')) {
	        	$this->_isgroup = true;
	        	$this->_groupitemwidth = $catdetail->getData('kalolia_subcat_colw');
	        } else {
	        	$this->_isgroup = false;
	        }
        }
        ...
        if($level==0){
	        $menutypes = $catdetail->getData('kalolia_menutype');
	        $rightblock = $this->_getCatBlock($catdetail, 'kalolia_block_r');
	        $topblock = $this->_getCatBlock($catdetail, 'kalolia_block_t');
	        $bottomblock = $this->_getCatBlock($catdetail, 'kalolia_block_b');
	        $hidelink = $this->_getCatBlock($catdetail, 'kalolia_menulink');
        ...
        if ($level == 0) {
        	$submenuwidth = $catdetail->getData('kalolia_subcat_w');
		...
		if($level == 0){
	        ...
	        if($catdetail->getData('kalolia_label')) $html[] = '<span class="label">' . $catdetail->getData('kalolia_label') . '</span>';
	        $html[] = '<span>' . $this->escapeHtml($category->getName()) . '</span>';
	        $html[] = '</a>';
		} elseif($level == 1) {
			$html[] = '<li class="'.$liclass.'">';
    ...
    public function getMenuHtml($momenu = false, $level = 0) {
        ...
        $customitems	=	Mage::helper('kalolia/data')->getField('menu_customItems');
