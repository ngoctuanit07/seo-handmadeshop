...
class Sns_Kalolia_Adminhtml_ConfigController extends Mage_Adminhtml_Controller_Action{ 
	...
	public function createBlocks() {
    	$pathEtc = Mage::getModuleDir('etc', 'Sns_Kalolia');
	    ...
	    foreach ($blockCollection as $cmsblock) :
	    	if (preg_match('/^kalolia_/i', $cmsblock->getIdentifier())) :
	...
	public function createPages() {
	    ...
	    foreach ($pageCollection as $cmspage) :
	    	if (preg_match('/^kalolia_/i', $cmspage->getIdentifier())) :
	...
	public function createConfig() {
		...
		$section = Mage::getStoreConfig('sns_kalolia_cfg');
		foreach($section as $group => $fields){
			foreach($fields as $field => $value){
				$xmlObj->setNode('default/sns_kalolia_cfg/'.$group.'/'.$field, '<![CDATA['.$value.']]>');
			}
		}
	...
	function removeConfig () {
		...
		$dom = new DOMDocument();
		$dom->load($xmlPath);
		$configs = $dom->getElementsByTagName('sns_kalolia_cfg'); //translate
