...
{
    const ITEM_TYPE_PRODUCT_URL      = 'http://schema.org/Product';
     ...
     * @return string
     */
    public function getRegExp($cssSelector, &$matchNum, $additional = '')
    {
        ...
        for ($i = 0; $i < count($selectorChain); $i ++) {
            ...
            if ($i != count($selectorChain) - 1) {
                ...
                if ($selectorName) {
                    $rexExp .= "(\<\w+[^>]*?$selectorName=['|\"]((?=.).*?\s|){$selectorClass}((?=.)\s.*?|)['|\"][\s\S]*?)(\/?\>){$additional}/i";
                } else {
                    //add unnecessary brackets for keep numbers of replacement
                    $rexExp .= "(\<{$selectorClass}((?=.)\s.*?|)[\s\S]*?(.)*?)(\/?\>){$additional}/i";
                }
            }
        }
